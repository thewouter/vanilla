...
class DiscussionEventPlugin extends Gdn_Plugin {
	...
	public function DiscussionsController_AfterPageTitle_handler($Sender){/*
		echo "Eerst komende Opkomsten";
		$DiscussionEventModule = new DiscussionEventModule($Sender);
		$DiscussionEventModule->getData();
		var_dump($DiscussionEventModule);
		?> <ul class="DataList Discussions"><?php 
		foreach ($DiscussionEventModule->Data('DiscussionEvents')->Result() as $Discussion) {
			?>
			<li class="Item Mine Participated Read ItemDiscussion">
				<div class="ItemContent Discussion">
					<div class="Title">
					<?php echo Anchor(Gdn_Format::Text($Discussion->Name . " (".$Discussion->DiscussionEventDate.")", false), DiscussionUrl($Discussion).($Discussion->CountCommentWatch > 0 ? '#Item_'.$Discussion->CountCommentWatch : ''), 'DiscussionLink'); ?>
					
						<?php self::displayEventDate($Discussion->DiscussionEventDate); ?>
					<div class="Meta"><span class="MItem">
					</span></div></div></div>
			</li>
			<?php
		}
